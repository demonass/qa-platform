      ...
      <div className="sticky bottom-0 border-t border-border/50 bg-background/80 p-4 backdrop-blur-xl">
        <div className="mx-auto max-w-3xl">
          <div className="mb-2 flex items-center justify-center gap-2">
            <Button variant={webSearchMode ? 'default' : 'outline'} size="sm"
              className={cn('gap-2 transition-all', webSearchMode ? 'bg-blue-500 hover:bg-blue-600 text-white' : 'bg-transparent hover:bg-muted')}
              onClick={onWebSearchToggle}>
              <Globe className={cn('size-4', webSearchMode ? 'text-white' : '')} />
              <span className="text-sm">{webSearchMode ? '🌐 联网搜索已开启' : '🔍 开启联网搜索'}</span>
            </Button>
            {webSearchMode && <span className="text-xs text-muted-foreground">模型将可以访问互联网获取最新信息</span>}
          </div>
          <div className="relative flex items-end gap-2 rounded-2xl border border-border bg-card p-2 shadow-sm transition-shadow focus-within:shadow-md focus-within:ring-1 focus-within:ring-ring/20">
            <Textarea ref={textareaRef} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown}
              placeholder="输入消息..."
              className={cn('min-h-[44px] max-h-[200px] flex-1 resize-none border-0 bg-transparent px-3 py-3', 'placeholder:text-muted-foreground/60 focus-visible:ring-0 focus-visible:ring-offset-0')}
              rows={1} />
            <Button onClick={isLoading ? onStop : handleSubmit} disabled={!input.trim() && !isLoading} size="icon"
              className={cn('size-10 shrink-0 rounded-xl transition-all',
          ...
          <p className="mt-2 text-center text-xs text-muted-foreground">AI 可能会产生不准确的信息。请验证重要内容。</p>
        </div>
      </div>
    )
  }
